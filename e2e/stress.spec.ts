...
async function processTreeRss(pid: number): Promise<number> {
  const seen = new Set<number>()
  const visit = async (id: number): Promise<number> => {
    if (seen.has(id)) return 0
    seen.add(id)
    try {
      const [status, children] = await Promise.all([
        readFile(`/proc/${id}/status`, 'utf8'),
        readFile(`/proc/${id}/task/${id}/children`, 'utf8'),
      ])
      const rss = Number(status.match(/^VmRSS:\s+(\d+)/m)?.[1] ?? 0) * 1024
      return rss + (await Promise.all(children.trim().split(/\s+/).filter(Boolean).map(Number).map(visit))).reduce((sum, value) => sum + value, 0)
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT' || (error as NodeJS.ErrnoException).code === 'ESRCH') return 0
      throw error
    }
  }
  return visit(pid)
...
test('sustained real-document workload records latency, memory, queue pressure and durable content', async () => {
  ...
  const inputs = await Promise.all([largeSource, editSource, resolve('README.md'), resolve('SECURITY.md')].map(async (path) => {
    ...
    return { name: basename(path), bytes: bytes.length, sha256: createHash('sha256').update(bytes).digest('hex') }
  }))
  const application = await launch(directory, [largePath, editPath, recentPath])
  const rss: Array<{ elapsedMs: number; bytes: number }> = []
  ...
  const sampler = (async () => {
    while (sampling) {
      try { rss.push({ elapsedMs: Date.now() - started, bytes: await processTreeRss(application.process.pid!) }) }
  ...
  try {
    ...
    report.processTreeRss = { peakBytes: Math.max(0, ...rss.map((sample) => sample.bytes)), samples: rss }
    ...
    if (sampleError) report.samplingError = String(sampleError)
